...
const RULE_VIOLATION_MESSAGES: RuleMessage[] = [
  ...
  { match: "Campo obrigatório ausente: data_reuniao.", message: "Informe a data da reunião." },
  { match: "Campo obrigatório vazio: data_reuniao.", message: "A data da reunião não pode ficar em branco." },
  ...
  { match: "Data/hora inválida para data_reuniao. Use ISO 8601.", message: "A data da reunião está inválida. Escolha uma data válida." },
  ...
  { match: "Reuniões futuras devem permanecer como Agendada.", message: "Reuniões com data futura só podem ficar com status Agendada." },
  ...
  { match: "Nome de servico vinculado a vendas nao pode ser alterado.", message: "O nome de um serviço já usado em vendas não pode ser alterado. Cadastre um novo serviço com o nome desejado." },
  { match: "Valor de servico vinculado a vendas nao pode ser alterado.", message: "O valor de um serviço já usado em vendas não pode ser alterado." },
  { match: "Serviço vendido só pode ser excluído quando todos os leads vinculados estiverem Inativos.", message: "Este serviço só pode ser excluído quando todas as vendas vinculadas pertencerem a leads Inativos." },
...
function canonicalRuleKey(value: string) {
...
const DIRECT_MESSAGE_MAP = new Map<string, string>([
  [
    canonicalRuleKey("Lead só pode ser criado como Em prospecção após existir tentativa de contato."),
  ...
  [
    canonicalRuleKey("Lead só pode ser criado como Cliente após existir venda registrada."),
    "Este lead não pode ser salvo com situação Cliente, porque ainda não existe nenhuma venda vinculada a ele.",
  ],
  [
    canonicalRuleKey("Lead só pode virar Em prospecção quando existir tentativa de contato."),
    "Este lead não pode ficar com situação Em prospecção, porque ainda não existe nenhuma tentativa de contato vinculada a ele.",
  ],
  [
    canonicalRuleKey("Lead inativo não pode receber novos registros de tentativa, reunião ou venda."),
...
function resolveMappedMessage(rawMessage: string) {
  ...
  const canonicalRawMessage = canonicalRuleKey(rawMessage);

  const directMatch = DIRECT_MESSAGE_MAP.get(canonicalRawMessage);
  if (directMatch) {
    return directMatch;
  }

  if (/lead so pode ser criado como em prospeccao/i.test(canonicalRawMessage)) {
  ...
  if (/lead so pode virar em prospeccao/i.test(canonicalRawMessage)) {
  ...
  if (/lead so pode ser criado como cliente/i.test(canonicalRawMessage)) {
    return "Este lead não pode ser salvo com situação Cliente, porque ainda não existe nenhuma venda vinculada a ele.";
  }
  if (/violates check constraint/i.test(canonicalRawMessage) && /tem_site/i.test(canonicalRawMessage)) {
    return "O campo 'tem site' aceita apenas SIM ou NÃO.";
  }

  for (const rule of RULE_VIOLATION_MESSAGES) {
    if (typeof rule.match === "string" && canonicalRuleKey(rule.match) === canonicalRawMessage) {
      return rule.message;
    }
    if (rule.match instanceof RegExp && rule.match.test(canonicalRawMessage)) {
      return rule.message;
    }
  }
