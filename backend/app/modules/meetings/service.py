    ...
    def update_meeting(self, meeting_id: int, payload: dict) -> dict:
        with self.db.transaction():
            ...
            if "status_reuniao" in payload:
                ...
                if canonical_text_key(current_status) == canonical_text_key("Realizada") and canonical_text_key(new_status) in {
                    canonical_text_key("Remarcada"),
                    canonical_text_key("Não Compareceu"),
                }:
                    raise AppError("Reuniao realizada nao pode retroceder.")
                if canonical_text_key(current_status) == canonical_text_key("Não Compareceu") and canonical_text_key(new_status) == canonical_text_key("Realizada"):
